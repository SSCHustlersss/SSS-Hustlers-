...
async function loadTestsForSubject(subjectKey, label, icon, topicKey, inTopics) {
  ...
  if ((!data || data.length === 0) && topicKey) {
  ...
  document.getElementById(targetEl).innerHTML = '<div class="test-list">' +
    tests.map(([name, info], i) => {
      ...
      if (isLocked) {
        ...
        // FIX Bug #7: Stale localStorage clear karo agar Supabase mein attempt hai
        if (savedState && attemptCount > 0) { clearQuizState(name); }
...
async function submitQuiz() {
  ...
  try {
    ...
    // FIX Bug #18: Insert success hone pe hi localStorage clear karo
    if (!insertError) {
      clearQuizState(activeTestName);
    } else {
